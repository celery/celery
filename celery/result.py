...
class AsyncResult(ResultBase):
    ...
    def revoke_by_stamped_header(self, header, connection=None, terminate=False, signal=None,
        ...
        """Send revoke signal to all workers only for tasks with matching a header (or list of key-value pairs).
        ...
        Arguments:
            header (Union(dict, list[str])): Header fields to match.
            terminate (bool): Also terminate the process currently working
                on the task (if any).
            signal (str): Name of signal to send to process if terminate.
                Default is TERM.
            ...
            timeout (float): Time in seconds to wait for replies when
                ``wait`` is enabled.
        """
        self.app.control.revoke_by_stamped_header(header, connection=connection,
                                                  terminate=terminate, signal=signal,
                                                  reply=wait, timeout=timeout)
