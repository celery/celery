...
class RedisBackend(BaseKeyValueStoreBackend, AsyncBackendMixin):
    ...
    def _set_chunked_results(self, key, value, **retry_policy):
        # Normalize key to bytes to ensure bytes operations below are safe.
        if isinstance(key, str):
            ...
            raise BackendStoreError(
                f"Invalid key type for Redis backend: {type(key)!r}"
            )

        if self.task_keyprefix not in key_bytes:
            raise BackendStoreError(
                "Invalid key: Task Prefix must be in the key to use chunked results"
            )
        ...
        for index, chunk in enumerate(chunks):
            new_key = key_bytes.replace(
                self.task_keyprefix,
                b"task-chunk-" + str_to_bytes(str(index)),
            )
        ...
        return self.ensure(self._set, (key_bytes, merged_value), **retry_policy)
