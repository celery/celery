...
try:
    ...
    from kombu.transport.redis import get_redis_error_classes as get_valkey_error_classes
    from valkey._parsers.url_parser import URL_QUERY_ARGUMENT_PARSERS
except ImportError:
    valkey = None
    get_valkey_error_classes = None
...
E_VALKEY_MISSING = """
You need to install the valkey library in order to use \
the Valkey result store backend.
"""

E_VALKEY_SENTINEL_MISSING = """
You need to install the valkey library with support of \
sentinel in order to use the Valkey result store backend.
"""

W_VALKEY_SSL_CERT_OPTIONAL = """
Setting ssl_cert_reqs=CERT_OPTIONAL when connecting to valkey means that \
...
W_VALKEY_SSL_CERT_NONE = """
Setting ssl_cert_reqs=CERT_NONE when connecting to valkey means that celery \
will not validate the identity of the valkey broker when connecting. This \
leaves you vulnerable to man in the middle attacks.
"""

E_VALKEY_SSL_PARAMS_AND_SCHEME_MISMATCH = """
SSL connection parameters have been provided but the specified URL scheme \
is valkey://. A Valkey SSL connection URL should use the scheme valkeys://.
"""

E_VALKEY_SSL_CERT_REQS_MISSING_INVALID = """
A valkeys:// URL must have parameter ssl_cert_reqs and this must be set to \
CERT_REQUIRED, CERT_OPTIONAL, or CERT_NONE
"""

E_LOST = 'Connection to Valkey lost: Retry (%s/%s) %s.'

E_RETRY_LIMIT_EXCEEDED = """
Retry limit exceeded while trying to reconnect to the Celery valkey result \
...
class ValkeyBackend(BaseKeyValueStoreBackend, AsyncBackendMixin):
    ...
    valkey = valkey
    connection_class_ssl = valkey.SSLConnection if valkey else None
    ...
    def __init__(self, host=None, port=None, db=None, password=None,
        ...
        if self.valkey is None:
            raise ImproperlyConfigured(E_VALKEY_MISSING.strip())
        ...
        self.max_connections = (
            max_connections or
            _get('valkey_max_connections') or
            self.max_connections)
        self._ConnectionPool = connection_pool

        socket_timeout = _get('valkey_socket_timeout')
        socket_connect_timeout = _get('valkey_socket_connect_timeout')
        retry_on_timeout = _get('valkey_retry_on_timeout')
        socket_keepalive = _get('valkey_socket_keepalive')
        health_check_interval = _get('valkey_backend_health_check_interval')

        self.connparams = {
            'host': _get('valkey_host') or 'localhost',
            'port': _get('valkey_port') or 6379,
            'db': _get('valkey_db') or 0,
            'password': _get('valkey_password'),
        ...
        # valkey_backend_use_ssl dict, check ssl_cert_reqs is valid. If set
        # via query string ssl_cert_reqs will be a string so convert it here
        if ('connection_class' in self.connparams and
                issubclass(self.connparams['connection_class'], valkey.SSLConnection)):
            ...
            if ssl_cert_reqs not in ssl_string_to_constant.values():
                raise ValueError(E_VALKEY_SSL_CERT_REQS_MISSING_INVALID)

            if ssl_cert_reqs == CERT_OPTIONAL:
                logger.warning(W_VALKEY_SSL_CERT_OPTIONAL)
            elif ssl_cert_reqs == CERT_NONE:
                logger.warning(W_VALKEY_SSL_CERT_NONE)
            self.connparams['ssl_cert_reqs'] = ssl_cert_reqs

        self.url = url

        self.connection_errors, self.channel_errors = (
            get_valkey_error_classes() if get_valkey_error_classes
            else ((), ()))
    ...
    def _params_from_url(self, url, defaults):
        ...
        if scheme == 'socket':
            ...
            connparams.update({
                'connection_class': self.valkey.UnixDomainSocketConnection,
                'path': '/' + path,
            })
        ...
        if scheme == 'valkey':
            ...
            if (any(key in connparams for key in ssl_param_keys) or
                ...
                raise ValueError(E_VALKEY_SSL_PARAMS_AND_SCHEME_MISMATCH)

        if scheme == 'valkeys':
            connparams['connection_class'] = valkey.SSLConnection
        ...
        for key, value in query.items():
            if key in URL_QUERY_ARGUMENT_PARSERS:
                query[key] = URL_QUERY_ARGUMENT_PARSERS[key](
                    value
                )
    ...
    def set(self, key, value, **retry_policy):
        if isinstance(value, str) and len(value) > self._MAX_STR_VALUE_SIZE:
            raise BackendStoreError('value too large for Valkey backend')
    ...
    def _get_client(self):
        return self.valkey.StrictValkey
    ...
    def ConnectionPool(self):
        if self._ConnectionPool is None:
            self._ConnectionPool = self.valkey.ConnectionPool
...
class SentinelBackend(ValkeyBackend):
    ...
    def __init__(self, *args, **kwargs):
        if self.sentinel is None:
            raise ImproperlyConfigured(E_VALKEY_SENTINEL_MISSING.strip())
