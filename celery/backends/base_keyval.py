...
from ..result import GroupResult, allow_join_result
...
E_MISSING = """
You need to install the {} library in order to use \
the Valkey result store backend.
"""

E_SENTINEL_MISSING = """
You need to install the {} library with support of \
sentinel in order to use the Valkey result store backend.
"""

W_SSL_CERT_OPTIONAL = """
Setting ssl_cert_reqs=CERT_OPTIONAL when connecting to {} means that \
celery might not validate the identity of the valkey broker when connecting. \
This leaves you vulnerable to man in the middle attacks.
"""

W_SSL_CERT_NONE = """
Setting ssl_cert_reqs=CERT_NONE when connecting to {0} means that celery \
will not validate the identity of the {0} broker when connecting. This \
leaves you vulnerable to man in the middle attacks.
"""

E_SSL_PARAMS_AND_SCHEME_MISMATCH = """
SSL connection parameters have been provided but the specified URL scheme \
is {0}://. A Valkey SSL connection URL should use the scheme {0}s://.
"""

E_SSL_CERT_REQS_MISSING_INVALID = """
A {}s:// URL must have parameter ssl_cert_reqs and this must be set to \
CERT_REQUIRED, CERT_OPTIONAL, or CERT_NONE
"""

E_LOST = 'Connection to {} lost: Retry (%s/%s) %s.'
...
class BaseKeyValueBackend(BaseKeyValueStoreBackend, AsyncBackendMixin):
    ResultConsumer = ResultConsumer

    backend = None
    backend_module = None
    backend_name = ""
    connection_class_ssl = backend_module.SSLConnection if backend else None
    ...
    def __init__(self, host=None, port=None, db=None, password=None,
        ...
        if self.backend is None:
            raise ImproperlyConfigured(E_MISSING.format(self.backend_name))
        ...
        self.max_connections = (
            max_connections or
            _get(f"{self.backend_name}_max_connections") or
            self.max_connections
        )
        self._ConnectionPool = connection_pool

        socket_timeout = _get(f'{self.backend_name}_socket_timeout')
        socket_connect_timeout = _get(f'{self.backend_name}_socket_connect_timeout')
        retry_on_timeout = _get(f'{self.backend_name}_retry_on_timeout')
        socket_keepalive = _get(f'{self.backend_name}_socket_keepalive')
        health_check_interval = _get(f'{self.backend_name}_backend_health_check_interval')

        self.connparams = {
            'host': _get(f'{self.backend_name}_host') or 'localhost',
            'port': _get(f'{self.backend_name}_port') or 6379,
            'db': _get(f'{self.backend_name}_db') or 0,
            'password': _get(f'{self.backend_name}_password'),
        ...
        username = _get(f'{self.backend_name}_username')
        if username:
        ...
        # absent in redis\valkey.connection.UnixDomainSocketConnection
        if socket_keepalive:
            self.connparams['socket_keepalive'] = socket_keepalive

        # f"{self.backend_name}_backend_use_ssl" must be a dict with the keys:
        # 'ssl_cert_reqs', 'ssl_ca_certs', 'ssl_certfile', 'ssl_keyfile'
        # (the same as "broker_use_ssl")
        ssl = _get(f'{self.backend_name}_backend_use_ssl')
        if ssl:
        ...
        if ('connection_class' in self.connparams and
                issubclass(self.connparams['connection_class'], self.backend_module.SSLConnection)):
            ...
            if ssl_cert_reqs not in ssl_string_to_constant.values():
                raise ValueError(E_SSL_CERT_REQS_MISSING_INVALID.format(self.backend_name))

            if ssl_cert_reqs == CERT_OPTIONAL:
                logger.warning(W_SSL_CERT_OPTIONAL.format(self.backend_name))
            elif ssl_cert_reqs == CERT_NONE:
                logger.warning(W_SSL_CERT_NONE.format(self.backend_name))
    ...
    def _params_from_url(self, url, defaults):
        ...
        if scheme == 'socket':
            ...
            connparams.update({
                'connection_class': self.backend.UnixDomainSocketConnection,
                'path': '/' + path,
            })
        ...
        if scheme == self.backend_name:
            ...
            if (any(key in connparams for key in ssl_param_keys) or
                ...
                raise ValueError(self.SSL_PARAMS_AND_SCHEME_MISMATCH)

        if scheme == f'{self.backend_name}s':
            connparams['connection_class'] = self.backend_module.SSLConnection
        ...
        for key, value in query.items():
            if key in self.backend_module.connection.URL_QUERY_ARGUMENT_PARSERS:
                query[key] = self.backend_module.connection.URL_QUERY_ARGUMENT_PARSERS[key](
                    value
                )
    ...
    def on_connection_error(self, max_retries, exc, intervals, retries):
        tts = next(intervals)
        logger.error(
            E_LOST.format(self.backend_name.title()).strip(),
            retries, max_retries or 'Inf', humanize_seconds(tts, 'in '))
        return tts

    def set(self, key, value, **retry_policy):
        if isinstance(value, str) and len(value) > self._MAX_STR_VALUE_SIZE:
            raise BackendStoreError(f'value too large for {self.backend_name.title()} backend')
    ...
    def _get_client(self):
        return getattr(self.backend, f"Strict{self.backend_name.title()}")
    ...
    def ConnectionPool(self):
        if self._ConnectionPool is None:
            self._ConnectionPool = self.backend.ConnectionPool
...
class BaseSentinelBackend:
    ...
    def __init__(self, *args, **kwargs):
        if self.sentinel is None:
            raise ImproperlyConfigured(E_SENTINEL_MISSING.format(self.backend_name.title()).strip())
    ...
    def _get_pool(self, **params):
        ...
        conf = {"service_name": master_name, f"{self.backend_name}_class": self._get_client()}
        return sentinel_instance.master_for(**conf).connection_pool
