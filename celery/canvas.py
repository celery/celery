...
class Signature(dict):
    ...
    chord_size = getitem_property("chord_size", "Size of chord (if applicable)")
...
class _basemap(Signature):
    ...
    def from_dict(cls, d, app=None):
        return _upgrade(
            d,
            cls(*cls._unpack_args(d["kwargs"]), app=app, **d["options"]),
        )
...
class chunks(Signature):
    ...
    def from_dict(cls, d, app=None):
        return _upgrade(
            d,
            chunks(*cls._unpack_args(d["kwargs"]), app=app, **d["options"]),
        )
    ...
    def apply_async(
        ...
        **options,
        ...
        if link_error is not None:
            raise TypeError("Cannot add link to group: do that on individual tasks")
        app = self.app
        if app.conf.task_always_eager:
            return self.apply(args, kwargs, **options)
        if not self.tasks:
