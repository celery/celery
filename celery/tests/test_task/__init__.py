...
class MonthlyPeriodic(task.PeriodicTask):
    """Second Thursday of any month"""
    run_every = crontab(hour=7, minute=30,
        day_of_week="thursday", day_of_month="8-14")


class YearlyPeriodic(task.PeriodicTask):
    """Second Thursday in March"""
    run_every = crontab(hour=7, minute=30,
        day_of_week="thursday", day_of_month="8-14", month_of_year=3)
...
class test_crontab_is_due(Case):
    ...
    @patch_crontab_nowfun(MonthlyPeriodic, datetime(2010, 5, 13, 7, 30))
    def test_monthly_execution_is_due(self):
        due, remaining = MonthlyPeriodic().is_due(datetime(2010, 4, 8, 7, 30))
        self.assertTrue(due)
        self.assertEqual(remaining, 28 * 24 * 60 * 60)

    @patch_crontab_nowfun(MonthlyPeriodic, datetime(2010, 5, 9, 10, 30))
    def test_monthly_execution_is_not_due(self):
        due, remaining = MonthlyPeriodic().is_due(datetime(2010, 4, 8, 7, 30))
        self.assertFalse(due)
        self.assertEqual(remaining, 4 * 24 * 60 * 60 - 3 * 60 * 60)

    @patch_crontab_nowfun(YearlyPeriodic, datetime(2010, 3, 11, 7, 30))
    def test_yearly_execution_is_due(self):
        due, remaining = YearlyPeriodic().is_due(datetime(2009, 3, 12, 7, 30))
        self.assertTrue(due)
        self.assertEqual(remaining, 364 * 24 * 60 * 60)

    @patch_crontab_nowfun(YearlyPeriodic, datetime(2010, 3, 7, 10, 30))
