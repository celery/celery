...
def run_benchmarks():
    ...
    print("Tracked task completion (with increment_group_progress):")
    print("  Backend calls: 1 (EVAL)")
    print("  Internal Lua operations: 5 (HEXISTS, SISMEMBER, SADD, HINCRBY, TTL)")
    print(f"  Full mock breakdown: {increment_counts}")
    print()
    print("Untracked task completion (group never initialized for tracking):")
    print("  Backend calls: 1 (EVAL)")
    print("  Initialization check: HEXISTS (returns false, exits early)")
    print("  Progress bookkeeping writes: 0")
    print(f"  Full mock breakdown: {untracked_counts}")
    print()
    print("Additional overhead for tracking: 1 EVAL backend call")
    print("  (The Lua script performs 5 internal Redis operations for idempotent bookkeeping)")
    print()
    ...
    if native_results[2][1] > 0:
        ...
        print("  - Native path not working (0 calls detected)")
    print()
