...
class test_link_error:
    ...
    def test_link_error_callback_error_callback_retries_eager(self):
        exception = ExpectedException("Task expected to fail", "test")
        result = fail.apply(args=("test",), link_error=retry_once.s(countdown=None))
    ...
    def test_link_error_callback_retries(self):
        ...
        result = fail.apply_async(
            args=("test",), link_error=retry_once.s(countdown=None)
        )
        assert result.get(timeout=TIMEOUT, propagate=False) == exception
...
class test_chain:
    ...
    def test_chain_inside_group_receives_arguments(self, manager):
        c = add.s(5, 6) | group((add.s(1) | add.s(2), add.s(3)))
        res = c()
    ...
    def test_chord_soft_timeout_recuperation(self, manager):
        ...
        c = chord(
            [
                ...
                delayed_sum_with_soft_guard.s([100], pause_time=2).set(
                    soft_time_limit=1
                ),
            ]
        )
    ...
    def test_replace_chain_with_empty_chain(self, manager):
        ...
        with pytest.raises(
            ImproperlyConfigured, match="Cannot replace with an empty chain"
        ):
...
class test_group:
    ...
    def test_parent_ids(self, manager):
        ...
        g = ids.si(i=1) | ids.si(i=2) | group(ids.si(i=i) for i in range(2, 50))
        res = g()
...
class test_chord:
    ...
    def test_simple_chord_with_a_delay_in_group_save(self, manager, monkeypatch):
        ...
        monkeypatch.setattr(
            BaseKeyValueStoreBackend, "_apply_chord_incr", apply_chord_incr_with_sleep
        )
    ...
    def test_redis_subscribed_channels_leak(self, manager):
        ...
        assert (
            channels_before_count
            <= chord_header_task_count * total_chords + initial_channels_count
        )

        result_values = [result.get(timeout=TIMEOUT) for result in async_results]
    ...
    def test_group_chain(self, manager):
        ...
        c = add.s(2, 2) | group(add.s(i) for i in range(4)) | add_to_all.s(8)
        res = c()
    ...
    def test_nested_chord(self, manager):
        ...
        c = group(
            [
                group([group([group([add.s(1, 2)]) | add.s([3])]) | add.s([4])])
                | add.s([5])
            ]
        ) | add.s([6])
    ...
    def assert_parentids_chord(self, res, expected_root_id):
        ...
        # ids(i=1)
        root_id, parent_id, value = res.parent.parent.parent.parent.get(timeout=30)
        assert value == 1
        assert root_id == expected_root_id
    ...
    def test_chord_on_error(self, manager):
        ...
        c1 = chord(
            ...
            body=print_unicode.s("This should not be called").on_error(chord_error.s()),
        )
        res = c1()
        with pytest.raises(ExpectedException):
            res.get(propagate=True)
        ...
        callback_chord_exc = AsyncResult(res.children[0].children[0].result[0]).result
        ...
        assert [cr[3] for cr in chord_results if cr[2] == states.SUCCESS] == [3, 7]

        assert len([cr for cr in chord_results if cr[2] != states.SUCCESS]) == 1
        ...
        c = chain(
            ...
            chord(
                [
                    ...
                    chain(
                        identity.si(3),
                        chord([identity.si(4), identity.si(5)], identity.si(6)),
                    ),
                ],
                identity.si(7),
            ),
...
class test_signature_serialization:
    ...
    def test_rebuild_nested_chain_chain(self, manager):
        sig = chain(
            tasks.return_nested_signature_chain_chain.s(), tasks.rebuild_signature.s()
        )
        sig.delay().get(timeout=TIMEOUT)

    def test_rebuild_nested_chain_group(self, manager):
        sig = chain(
            tasks.return_nested_signature_chain_group.s(), tasks.rebuild_signature.s()
        )
        sig.delay().get(timeout=TIMEOUT)
    ...
    def test_rebuild_nested_chain_chord(self, manager):
        ...
        sig = chain(
            tasks.return_nested_signature_chain_chord.s(), tasks.rebuild_signature.s()
        )
        sig.delay().get(timeout=TIMEOUT)

    def test_rebuild_nested_group_chain(self, manager):
        sig = chain(
            tasks.return_nested_signature_group_chain.s(), tasks.rebuild_signature.s()
        )
        sig.delay().get(timeout=TIMEOUT)

    def test_rebuild_nested_group_group(self, manager):
        sig = chain(
            tasks.return_nested_signature_group_group.s(), tasks.rebuild_signature.s()
        )
        sig.delay().get(timeout=TIMEOUT)
    ...
    def test_rebuild_nested_group_chord(self, manager):
        ...
        sig = chain(
            tasks.return_nested_signature_group_chord.s(), tasks.rebuild_signature.s()
        )
        sig.delay().get(timeout=TIMEOUT)
    ...
    def test_rebuild_nested_chord_chain(self, manager):
        ...
        sig = chain(
            tasks.return_nested_signature_chord_chain.s(), tasks.rebuild_signature.s()
        )
        sig.delay().get(timeout=TIMEOUT)
    ...
    def test_rebuild_nested_chord_group(self, manager):
        ...
        sig = chain(
            tasks.return_nested_signature_chord_group.s(), tasks.rebuild_signature.s()
        )
        sig.delay().get(timeout=TIMEOUT)
    ...
    def test_rebuild_nested_chord_chord(self, manager):
        ...
        sig = chain(
            tasks.return_nested_signature_chord_chord.s(), tasks.rebuild_signature.s()
        )
        sig.delay().get(timeout=TIMEOUT)
