...
class test_chain:
    ...
    @flaky
    def test_group_results_in_chain(self):
    ...
    def test_chain_on_error(self):
        # Run the chain and wait for the error callback to finish.
        c1 = chain(
    ...
    @flaky
    def test_eager_chain_inside_task(self):
    ...
    @flaky
    def test_group_chord_group_chain(self, redis_connection):
    ...
    @flaky
    def test_group_result_not_has_cache(self):
        t1 = identity.si(1)
        t2 = identity.si(2)
    ...
    @flaky
    def test_second_order_replace(self, redis_connection):
        redis_connection.delete('redis-echo')
    ...
    def test_replace_chain_with_empty_chain(self):
...
@pytest.mark.usefixtures("manager", "requires_chord_support")
class test_chain_chord_uplift:
    @pytest.mark.usefixtures("requires_redis_backend")
    def test_chord_soft_timeout_recuperation(self):
    ...
    def test_chain_error_handler_with_eta(self):
    ...
    @flaky
    def test_groupresult_serialization(self):
    ...
    @flaky
    def test_chain_of_task_a_group_and_a_chord(self):
        c = add.si(1, 0)
    ...
    @flaky
    def test_chain_of_chords_with_two_tasks(self):
        c = add.si(1, 0)
        c = c | group(add.s(1), add.s(1))
        c = c | tsum.s()
        c = c | add.s(1)
    ...
    @flaky
    def test_chain_of_a_chord_and_a_group_with_two_tasks(self):
        c = add.si(1, 0)
        c = c | group(add.s(1), add.s(1))
        c = c | tsum.s()
        c = c | add.s(1)
    ...
    @flaky
    def test_chain_of_a_chord_and_a_task_and_a_group(self):
    ...
    @flaky
    def test_chain_of_a_chord_and_two_tasks_and_a_group(self):
        c = group(add.s(1, 1), add.s(1, 1))
        c = c | tsum.s()
        c = c | add.s(1)
        c = c | add.s(1)
    ...
    @flaky
    def test_chain_of_a_chord_and_three_tasks_and_a_group(self):
        c = group(add.s(1, 1), add.s(1, 1))
        c = c | tsum.s()
        c = c | add.s(1)
        c = c | add.s(1)
    ...
    @flaky
    def test_nested_chain_group_lone(self):
        """
    ...
    def test_nested_chain_group_mid(self):
        """
    ...
    def test_nested_chain_group_last(self):
        """
...
@pytest.mark.usefixtures("manager", "requires_redis_backend")
class test_chain_links:
    def test_chain_replaced_with_a_chain_and_a_callback(
    ...
    def test_chain_with_eb_replaced_with_chain_with_eb(
        self, subtests, redis_connection,
    ):
        redis_connection.delete('redis-echo')
    ...
    def test_chain_children_with_callbacks(
        self, subtests, redis_connection, unique_redis_key,
    ):
        callback = redis_count.si(redis_key=unique_redis_key)
        ...
        with subtests.test(msg="Chain child task callbacks are called"):
            await_redis_count(child_task_count, redis_key=unique_redis_key)

    def test_chain_children_with_errbacks(
        self, subtests, redis_connection, unique_redis_key,
    ):
        errback = redis_count.si(redis_key=unique_redis_key)
        ...
        with subtests.test(msg="Chain child task errbacks are called"):
            ...
            await_redis_count(1, redis_key=unique_redis_key)

    def test_chain_with_callback_child_replaced(
        self, subtests, redis_connection, unique_redis_key,
    ):
        callback = redis_count.si(redis_key=unique_redis_key)
    ...
    def test_chain_with_callback_kwarg_child_replaced(
        self, subtests, redis_connection, unique_redis_key,
    ):
        callback = redis_count.si(redis_key=unique_redis_key)

        chain_sig = chain(
            (add_replaced.si(42, 1337), identity.s(), ),
            link=callback,
        )
    ...
    def test_chain_with_errback_child_replaced(
        self, subtests, redis_connection, unique_redis_key,
    ):
        errback = redis_count.si(redis_key=unique_redis_key)
    ...
    def test_chain_child_with_callback_replaced(
        self, subtests, redis_connection, unique_redis_key,
    ):
        callback = redis_count.si(redis_key=unique_redis_key)
    ...
    def test_chain_child_with_errback_replaced(
        self, subtests, redis_connection, unique_redis_key,
    ):
        errback = redis_count.si(redis_key=unique_redis_key)
...
class test_group:
    ...
    def test_nested_group_group(self):
        """
    ...
    @pytest.mark.usefixtures("requires_chord_support")
    def test_nested_group_chord_counting_simple(self):
    ...
    @pytest.mark.usefixtures("requires_chord_support")
    def test_nested_group_chord_counting_chain(self):
        gchild_count = 42
    ...
    @pytest.mark.usefixtures("requires_chord_support")
    def test_nested_group_chord_counting_group(self):
        gchild_count = 42
    ...
    @pytest.mark.usefixtures("requires_chord_support")
    def test_nested_group_chord_counting_chord(self):
        gchild_count = 42
        gchild_sig = chord(
    ...
    @pytest.mark.usefixtures("requires_chord_support")
    def test_nested_group_chord_counting_mixed(self):
        gchild_count = 42
        child_chord = chord(
            (
                identity.si(42),
    ...
    @pytest.mark.usefixtures("requires_chord_support")
    def test_nested_group_chord_body_chain(self):
...
@pytest.mark.usefixtures("manager", "requires_redis_backend")
class test_group_links:
    def test_callback_called_by_group(
        self, subtests, redis_connection, unique_redis_key,
    ):
        callback_msg = str(uuid.uuid4()).encode()
        callback = redis_echo.si(callback_msg, redis_key=unique_redis_key)
        ...
        with subtests.test(msg="Callback is called after group is completed"):
            await_redis_echo({callback_msg, }, redis_key=unique_redis_key)

    def test_errback_called_by_group_fail_first(
        self, subtests, redis_connection, unique_redis_key,
    ):
        errback_msg = str(uuid.uuid4()).encode()
        errback = redis_echo.si(errback_msg, redis_key=unique_redis_key)
        ...
        with subtests.test(msg="Errback is called after group task fails"):
            await_redis_echo({errback_msg, }, redis_key=unique_redis_key)

    def test_errback_called_by_group_fail_last(
        self, subtests, redis_connection, unique_redis_key,
    ):
        errback_msg = str(uuid.uuid4()).encode()
        errback = redis_echo.si(errback_msg, redis_key=unique_redis_key)
    ...
    def test_errback_called_by_group_fail_multiple(
        self, subtests, redis_connection, unique_redis_key,
    ):
        ...
        with subtests.test(msg="Errback is called after group task fails"):
            await_redis_count(
                expected_errback_count, redis_key=unique_redis_key
            )

    def test_group_children_with_callbacks(
        self, subtests, redis_connection, unique_redis_key,
    ):
        callback = redis_count.si(redis_key=unique_redis_key)
    ...
    def test_group_children_with_errbacks(
        self, subtests, redis_connection, unique_redis_key
    ):
        errback = redis_count.si(redis_key=unique_redis_key)
    ...
    def test_group_with_callback_child_replaced(
        self, subtests, redis_connection, unique_redis_key,
    ):
        callback = redis_count.si(redis_key=unique_redis_key)
    ...
    def test_group_with_errback_child_replaced(
        self, subtests, redis_connection, unique_redis_key,
    ):
        errback = redis_count.si(redis_key=unique_redis_key)
    ...
    def test_group_child_with_callback_replaced(
        self, subtests, redis_connection, unique_redis_key,
    ):
        callback = redis_count.si(redis_key=unique_redis_key)
    ...
    def test_group_child_with_errback_replaced(
        self, subtests, redis_connection, unique_redis_key,
    ):
        errback = redis_count.si(redis_key=unique_redis_key)
...
class test_chord:
    ...
    @flaky
    def test_replaced_nested_chord(self):
        c1 = chord([
            chord(
    ...
    @flaky
    def test_add_chord_to_chord(self):
        c = group([add_chord_to_chord.s([1, 2, 3], 4)]) | identity.s()
        res = c()
        assert sorted(res.get()) == [0, 5 + 6 + 7]

    @flaky
    def test_eager_chord_inside_task(self):
    ...
    @flaky
    def test_single_task_header(self):
    ...
    def test_empty_header_chord(self):
    ...
    def test_chord_on_error(self, manager, redis_connection):
    ...
    @flaky
    def test_chord_in_chords_with_chains(self):
        c = chord(
            group([
                chain(
                    add.si(1, 2),
    ...
    def test_chord_in_chain_with_args(self):
        c1 = chain(
            chord(
    ...
    @flaky
    def test_chain_to_a_chord_with_large_header(self):
    ...
    def test_nested_chord_group(self):
        """
    ...
    def test_nested_chord_group_chain_group_tail(self):
        """
    ...
    def test_error_propagates_from_chord(self):
    ...
    def test_error_propagates_from_chord2(self):
    ...
    def test_error_propagates_to_chord_from_simple(self, subtests):
        child_sig = fail.s()
    ...
    def test_error_propagates_to_chord_from_chain(self, subtests):
    ...
    def test_error_propagates_to_chord_from_chain_tail(self, subtests):
    ...
    def test_error_propagates_to_chord_from_group(self, subtests):
    ...
    def test_chord_header_task_replaced_with_chain(self):
        orig_sig = chord(
    ...
    def test_chord_header_child_replaced_with_chain_first(self):
        orig_sig = chord(
    ...
    def test_chord_header_child_replaced_with_chain_middle(self):
        orig_sig = chord(
            (identity.s(42), replace_with_chain.s(1337), identity.s(31337), ),
            identity.s(),
        )
    ...
    def test_chord_header_child_replaced_with_chain_last(self):
        orig_sig = chord(
    ...
    def test_chord_body_task_replaced_with_chain(self):
        orig_sig = chord(
            identity.s(42),
    ...
    def test_chord_body_chain_child_replaced_with_chain_first(self):
        orig_sig = chord(
            identity.s(42),
    ...
    def test_chord_body_chain_child_replaced_with_chain_middle(self):
        orig_sig = chord(
            identity.s(42),
    ...
    def test_chord_body_chain_child_replaced_with_chain_last(self):
        orig_sig = chord(
            identity.s(42),
...
@pytest.mark.usefixtures("manager", "requires_redis_backend")
class test_chord_links:
    def test_errback_called_by_chord_from_simple(
        self, subtests, redis_connection, unique_redis_key,
    ):
        errback_msg = str(uuid.uuid4()).encode()
        errback = redis_echo.si(errback_msg, redis_key=unique_redis_key)
        ...
        with subtests.test(
            ...
            await_redis_echo({errback_msg, }, redis_key=unique_redis_key)
        redis_connection.delete(unique_redis_key)
    ...
    def test_errback_called_by_chord_from_chain(
        self, subtests, redis_connection, unique_redis_key,
    ):
        errback_msg = str(uuid.uuid4()).encode()
        errback = redis_echo.si(errback_msg, redis_key=unique_redis_key)
    ...
    def test_errback_called_by_chord_from_chain_tail(
        self, subtests, redis_connection, unique_redis_key,
    ):
        errback_msg = str(uuid.uuid4()).encode()
        errback = redis_echo.si(errback_msg, redis_key=unique_redis_key)
    ...
    def test_errback_called_by_chord_from_group(
        self, subtests, redis_connection, unique_redis_key,
    ):
        errback_msg = str(uuid.uuid4()).encode()
        errback = redis_echo.si(errback_msg, redis_key=unique_redis_key)
    ...
    def test_errback_called_by_chord_from_group_fail_multiple(
        ...
        with subtests.test(msg="Errback is called after body group fails"):
            ...
            await_redis_count(fail_task_count, redis_key=unique_redis_key)
...
class test_signature_serialization:
    ...
    def test_rebuild_nested_chain_chain(self):
        sig = chain(
    ...
    def test_rebuild_nested_chain_group(self):
        sig = chain(
    ...
    @pytest.mark.usefixtures("requires_chord_support")
    def test_rebuild_nested_chain_chord(self):
        sig = chain(
    ...
    def test_rebuild_nested_group_chain(self):
        sig = chain(
    ...
    def test_rebuild_nested_group_group(self):
        sig = chain(
    ...
    @pytest.mark.usefixtures("requires_chord_support")
    def test_rebuild_nested_group_chord(self):
        sig = chain(
    ...
    @pytest.mark.usefixtures("requires_chord_support")
    def test_rebuild_nested_chord_chain(self):
        sig = chain(
    ...
    @pytest.mark.usefixtures("requires_chord_support")
    def test_rebuild_nested_chord_group(self):
        sig = chain(
    ...
    @pytest.mark.usefixtures("requires_chord_support")
    def test_rebuild_nested_chord_chord(self):
        sig = chain(
