...
def Message(
    ...
    return Mock(
        ...
        properties={"correlation_id": isinstance(body, dict) and body["id"] or None},
    )
...
class test_start_filter:
    def test_start(self):
        with patch("celery.contrib.migrate.eventloop") as evloop:
            ...
            state = State()
            start_filter(
                app, conn, filt, tasks="add,mul", callback=cb, state=state, limit=1
            )
...
class test_migrate_tasks:
    def test_migrate(self, app, name="testcelery"):
        ...
        Producer(x).publish("baz", exchange=name, routing_key=name)
        migrate_tasks(
            x, y, callback=callback, migrate=migrate, accept=["text/plain"], app=app
        )
