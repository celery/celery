...
class test_LoaderBase:
    ...
    def test_read_configuration_no_env(self):
        assert (
            base.BaseLoader(app=self.app).read_configuration("FOO_X_S_WE_WQ_Q_WE")
            is None
        )
    ...
    def test_import_default_modules(self):
        ...
        assert sorted(modnames(self.loader.import_default_modules())) == sorted(
            modnames([os, sys])
        )
...
class test_DefaultLoader:
    ...
    def test_read_configuration_ImportError(self):
        ...
        prev, os.environ["CELERY_CONFIG_MODULE"] = (
            os.environ.get("CELERY_CONFIG_MODULE", sentinel),
            "daweqew.dweqw",
        )
        try:
            l = default.Loader(app=self.app)
