...
class mScheduler(beat.Scheduler):
    ...
    def send_task(self, name=None, args=None, kwargs=None, **options):
        self.sent.append(
            {"name": name, "args": args, "kwargs": kwargs, "options": options}
        )
...
class test_Scheduler:
    ...
    def test_apply_async_with_null_args(self):
        ...
        scheduler.apply_async(
            scheduler.Entry(task=foo.name, app=self.app, args=None, kwargs=None)
        )
        foo.apply_async.assert_called()
    ...
    def test_due_tick_SchedulingError(self, error):
        ...
        scheduler.add(name="test_due_tick_SchedulingError", schedule=always_due)
        assert scheduler.tick() == 0
        error.assert_called()
    ...
    def test_honors_max_interval(self):
        ...
        scheduler.add(
            name="test_honors_max_interval", schedule=mocked_schedule(False, maxi * 4)
        )
    ...
    def test_ticks(self):
        ...
        s = {
            "test_ticks%s" % i: {"schedule": mocked_schedule(False, j)}
    ...
    def test_schedule_no_remain(self):
        scheduler = mScheduler(app=self.app)
        scheduler.add(
            name="test_schedule_no_remain", schedule=mocked_schedule(False, None)
        )
    ...
    def test_merge_inplace(self):
        ...
        b.update_from_dict(
            {
                "bar": {"schedule": mocked_schedule(True, 40)},
    ...
    def test_when(self):
        ...
        now_time_casey = now_time_utc.astimezone(pytz.timezone("Antarctica/Casey"))
        scheduler = mScheduler(app=self.app)
        result_utc = scheduler._when(
            mocked_schedule(True, 10, lambda: now_time_utc), 10
        )
        result_casey = scheduler._when(
            mocked_schedule(True, 10, lambda: now_time_casey), 10
        )
    ...
    def test_populate_heap(self, _when):
        scheduler = mScheduler(app=self.app)
        scheduler.update_from_dict({"foo": {"schedule": mocked_schedule(True, 10)}})
        scheduler.populate_heap()
        assert scheduler._heap == [event_t(1, 5, scheduler.schedule["foo"])]

    def create_schedule_entry(
        self, schedule=None, args=(), kwargs={}, options={}, task=None
    ):
        entry = {
            "name": "celery.unittest.add",
            "schedule": schedule,
...
def create_persistent_scheduler_w_call_logging(shelv=None):
    ...
    class MockPersistentScheduler(beat.PersistentScheduler):
        ...
        def send_task(self, task=None, args=None, kwargs=None, **options):
            self.sent.append(
                {"task": task, "args": args, "kwargs": kwargs, "options": options}
            )
            return self.app.AsyncResult(uuid())
...
class test_PersistentScheduler:
    ...
    def test_run_all_due_tasks_after_restart(self):
        ...
        app_schedule = {
            "first_missed": {
                "schedule": crontab(minute="*/10", nowfun=now_func),
                "task": "first_missed",
            },
            "second_missed": {
                "schedule": crontab(minute="*/1", nowfun=now_func),
                "task": "second_missed",
            },
            "non_missed": {
                "schedule": crontab(minute="*/13", nowfun=now_func),
                "task": "non_missed",
            },
        }
        shelve["entries"] = {
