...
class RouteCase:
    ...
    def assert_routes_to_queue(
        self, queue, router, name, args=None, kwargs=None, options=None
    ):
        if options is None:
            options = {}
        if kwargs is None:
...
class test_lookup_route(RouteCase):
    ...
    def test_lookup_takes_first(self):
        set_queues(self.app, foo=self.a_queue, bar=self.b_queue)
        R = routes.prepare(
            ({self.mytask.name: {"queue": "bar"}}, {self.mytask.name: {"queue": "foo"}})
        )
    ...
    def test_lookup_paths_traversed(self):
        ...
        R = routes.prepare(
            ({"celery.xaza": {"queue": "bar"}}, {self.mytask.name: {"queue": "foo"}})
        )
        router = Router(self.app, R, self.app.amqp.queues)
