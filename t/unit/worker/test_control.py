...
class test_ControlPanel:
    ...
    def test_time_limit(self):
        ...
        r = panel.handle(
            "time_limit",
            arguments={"task_name": self.mytask.name, "hard": 30, "soft": 10},
        )
        ...
        r = panel.handle(
            "time_limit",
            arguments={"task_name": self.mytask.name, "hard": None, "soft": None},
        )
        ...
        r = panel.handle(
            "time_limit", arguments={"task_name": "248e8afya9s8dh921eh928", "hard": 30}
        )
        assert "error" in r
    ...
    def test_dump_tasks2(self):
        prev, control.DEFAULT_TASK_INFO_ITEMS = (control.DEFAULT_TASK_INFO_ITEMS, [])
        try:
            info = "\n".join(self.panel.handle("dump_tasks"))
            assert "mytask" in info
    ...
    def test_rate_limit_invalid_rate_limit_string(self):
        e = self.panel.handle(
            "rate_limit",
            arguments={"task_name": "tasks.add", "rate_limit": "x1240301#%!"},
        )
    ...
    def test_rate_limit(self):
        ...
        panel.handle(
            "rate_limit", arguments={"task_name": task.name, "rate_limit": "100/m"}
        )
    ...
    def test_rate_limit_nonexistant_task(self):
        self.panel.handle(
            "rate_limit",
            arguments={"task_name": "xxxx.does.not.exist", "rate_limit": "1000/s"},
        )
