...
class test_DynamoDBBackend:
    ...
    def test_get_client_time_to_live_called(
        ...
        backend = DynamoDBBackend(
            app=self.app, url="dynamodb://key:secret@test?ttl_seconds=30"
        )
        backend._get_client()
    ...
    def test_set_table_ttl_describe_time_to_live_fails_raises(self):
        ...
        mock_describe_time_to_live = (
            self.backend._client.describe_time_to_live
        ) = MagicMock()
        client_error = ClientError(
            {
                "Error": {
    ...
    def test_set_table_ttl_enable_when_disabled_succeeds(self):
        ...
        mock_update_time_to_live = (
            self.backend._client.update_time_to_live
        ) = MagicMock()
    ...
    def test_prepare_put_request(self):
        expected = {
            ...
            "Item": {
                "id": {"S": "abcdef"},
                "result": {"B": "val"},
                "timestamp": {"N": str(Decimal(self._static_timestamp))},
            },
        }
    ...
    def test_get(self):
        ...
        self.backend.client.get_item.assert_called_once_with(
            Key={"id": {"S": "1f3fab"}}, TableName="celery"
        )
    ...
    def test_set(self):
        ...
        with patch("celery.backends.dynamodb.time", self._mock_time):
            assert (
                self.backend._set_with_state(
                    sentinel.key, sentinel.value, states.SUCCESS
                )
                is None
            )
