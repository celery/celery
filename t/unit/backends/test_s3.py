...
class test_S3Backend:
    ...
    @patch("botocore.credentials.CredentialResolver.load_credentials")
    def test_with_no_credentials_in_config_attempts_to_load_credentials(
    ...
    @patch("botocore.credentials.CredentialResolver.load_credentials")
    def test_with_credentials_in_config_does_not_search_for_credentials(
        self, mock_load_credentials
    ):
    ...
    @pytest.mark.parametrize(
        "aws_region",
        [None, "us-east-1"],
        ids=["No given aws region", "Specific aws region"],
    )
    ...
    @pytest.mark.parametrize(
        ...
        ids=["No given endpoint url", "Custom endpoint url"],
    )
    @patch("celery.backends.s3.boto3")
