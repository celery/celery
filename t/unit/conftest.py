...
def celery_config():
    return {
        ...
        "mongodb_backend_settings": {
            ...
            "taskmeta_collection": (
                os.environ.get("MONGO_TASKMETA_COLLECTION") or "taskmeta_collection"
            ),
...
def sanity_stdouts(request):
    ...
    if isinstance(sys.stdout, (LoggingProxy, Mock)) or isinstance(
        sys.__stdout__, (LoggingProxy, Mock)
    ):
        raise RuntimeError(CASE_LOG_REDIRECT_EFFECT.format(this, "stdout"))
    if isinstance(sys.stderr, (LoggingProxy, Mock)) or isinstance(
        sys.__stderr__, (LoggingProxy, Mock)
    ):
...
def teardown():
    ...
    remaining_threads = [
        thread for thread in threading.enumerate() if thread.getName() != "MainThread"
    ]
...
def import_all_modules(
    name=__name__, file=__file__, skip=("celery.decorators", "celery.task")
):
