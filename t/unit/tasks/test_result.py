...
class test_AsyncResult:
    ...
    def test_hash(self):
        assert hash(self.app.AsyncResult("x0w991")) == hash(
            self.app.AsyncResult("x0w991")
        )
        assert hash(self.app.AsyncResult("x0w991")) != hash(
            self.app.AsyncResult("x1w991")
        )
    ...
    def test_ready(self):
        oks = (
            self.app.AsyncResult(self.task1["id"]),
    ...
    def test_del(self):
        with patch("celery.result.AsyncResult.backend") as backend:
            ...
            del result
            assert backend.remove_pending_result.called_once_with(result_clone)
...
class test_ResultSet:
    def test_resultset_repr(self):
        assert repr(
            self.app.ResultSet([self.app.AsyncResult(t) for t in ["1", "2", "3"]])
        )
...
class SimpleBackend(SyncBackendMixin):
    ...
    def get_many(self, *args, **kwargs):
        return (
            (id, {"result": i, "status": states.SUCCESS})
        ...
        ts = self.app.GroupResult(
            ...
            [
                MockAsyncResultSuccess(
                    uuid(), result="1.1", app=self.app, backend=backend
                ),
                self.app.GroupResult(
                    uuid(),
                    [
                        MockAsyncResultSuccess(
                            uuid(), result="2.1", app=self.app, backend=backend
                        ),
                        self.app.GroupResult(
                            uuid(),
                            [
                                MockAsyncResultSuccess(
                                    uuid(), result="3.1", app=self.app, backend=backend
                                ),
                                MockAsyncResultSuccess(
                                    uuid(), result="3.2", app=self.app, backend=backend
                                ),
                            ],
                        ),
                    ],
...
class test_pending_Group:
    def setup(self):
        self.ts = self.app.GroupResult(
            uuid(), [self.app.AsyncResult(uuid()), self.app.AsyncResult(uuid())]
        )
...
class test_tuples:
    ...
    def test_GroupResult_with_parent(self):
        ...
        result = self.app.GroupResult(
            uuid(), [self.app.AsyncResult(uuid()) for _ in range(10)], parent
        )
    ...
    def test_GroupResult_as_tuple(self):
        ...
        result = self.app.GroupResult(
            "group-result-1",
            [self.app.AsyncResult(f"async-result-{i}") for i in range(2)],
            parent,
        )
