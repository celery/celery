...
class TasksCase:
    def setup(self):
        ...
        def retry_task_auto_retry_with_single_new_arg(self, ret=None, **kwargs):
            if ret is None:
                return self.retry(
                    exc=Exception("I have filled now"), args=["test"], kwargs=kwargs
                )
            else:
                return ret

        self.retry_task_auto_retry_with_single_new_arg = (
            retry_task_auto_retry_with_single_new_arg
        )

        @self.app.task(bind=True, max_retries=3, iterations=0, shared=False)
        def retry_task_auto_retry_with_new_args(
            self, ret=None, place_holder=None, **kwargs
        ):
            if ret is None:
        ...
        def retry_task_auto_retry_exception_with_new_args(
            self, ret=None, place_holder=None, **kwargs
        ):
            if ret is None:
                return self.retry(
                    exc=Exception("I have filled"),
                    args=[place_holder, place_holder],
                    kwargs=kwargs,
                )
            else:
                return ret

        self.retry_task_auto_retry_exception_with_new_args = (
            retry_task_auto_retry_exception_with_new_args
        )
        ...
        @self.app.task(
            bind=True, max_retries=3, iterations=0, base=MockApplyTask, shared=False
        )
        ...
        @self.app.task(
            bind=True, autoretry_for=(HTTPError,), retry_backoff=True, shared=False
        )
        ...
        @self.app.task(
            bind=True, base=TaskWithRetry, retry_kwargs={"max_retries": 2}, shared=False
        )
        ...
        @self.app.task(
            bind=True, base=TaskWithRetry, retry_backoff_max=16, shared=False
        )
        ...
        self.task_replaced_by_other_task_with_autoretry = (
            task_replaced_by_other_task_with_autoretry
        )
...
class test_task_retries(TasksCase):
    ...
    def test_eager_retry_with_new_params(self):
        assert (
            self.retry_task_auto_retry_with_new_args.si(place_holder="test")
            .apply()
            .get()
            == "test"
        )
    ...
    def test_eager_retry_with_autoretry_for_exception(self):
        assert (
            self.retry_task_auto_retry_exception_with_new_args.si(place_holder="test")
            .apply()
            .get()
            == "test"
        )
    ...
    def test_retry_not_eager(self):
        ...
        try:
            ...
            try:
                with pytest.raises(Retry):
                    self.retry_task_mockapply.retry(
                        args=[4, 4], kwargs={"task_retries": 0}, exc=exc, throw=True
                    )
                assert self.retry_task_mockapply.applied
            finally:
                self.retry_task_mockapply.applied = 0
...
class test_tasks(TasksCase):
    ...
    def test_inherit_parent_priority_child_task(self):
        ...
        self.app.amqp.send_task_message.assert_called_with(
            ANY,
            "t.unit.tasks.test_tasks.task_called_by_other_task",
            ANY,
            priority=5,
            queue=ANY,
            serializer=ANY,
    ...
    def assert_next_task_data_equal(
        self,
        consumer,
        presult,
        task_name,
    ...
    def test_regular_task(self):
        ...
        with self.app.connection_or_acquire() as conn:
            ...
            # send_task
            sresult = self.app.send_task(
                self.mytask.name, kwargs={"name": "Elaine M. Benes"}
            )
            self.assert_next_task_data_equal(
                consumer,
                sresult,
