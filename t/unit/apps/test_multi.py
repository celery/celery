...
class test_multi_args:
    ...
    def test_parse(self, gethostname, mkdirs_mock):
        ...
        p.parse()
        it = multi_args(
            p, cmd="celery multi", append="*AP*", prefix="*P*", suffix="*S*"
        )
        nodes = list(it)
        ...
        nodes2 = list(
            multi_args(p, cmd="celery multi", append="", prefix="*P*", suffix="*S*")
        )
        ...
        for i, worker in enumerate(nodes3[1:]):
            ...
            node_i = f"celery{i + 2}"
            assert worker.argv == ("celery multi", f"-n {node_i}@example.com") + _args(
                node_i
            )
        ...
        assert nodes4[0].argv == ("celery multi", "-c 5", "-n celery1@") + _args(
            "celery1"
        )
...
class test_Node:
    ...
    def test_waitexec(self, Popen, argv=["A", "B"]):
        ...
        Popen.assert_called_with(self.node.prepare_argv(argv, "python"), env=env)
    ...
    def test_handle_process_exit__signalled(self):
        on_signalled = Mock(name="on_signalled")
        assert self.node.handle_process_exit(-9, on_signalled=on_signalled) == 9
    ...
    def test_pidfile_custom(self, mock_exists, mock_dirs):
        n = Node.from_kwargs("foo@bar.com", pidfile="/var/run/demo/celery/%n.pid")
...
class test_Cluster:
    ...
    def test_start(self):
        ...
        self.cluster.start()
        self.cluster.start_node.assert_has_calls(call(node) for node in self.cluster)

    def test_start_node(self):
        self.cluster._start_node = Mock(name="_start_node")
        node = self.cluster[0]
        assert self.cluster.start_node(node) is self.cluster._start_node.return_value
